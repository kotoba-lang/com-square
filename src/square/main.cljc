(ns square.main
  "Kotodama WASM entrypoint for the Square clean-room actor (L4) — Clojure port.
  ...
  facts (`square.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "square")
(def tier "L4")
...
(def entity-specs
  [{:entity "Payment"      :plural "payments"      :id-prefix "square_pay"
    :fields [:orderId :customerId :amount :currency :status :sourceType] :required [:amount :currency]
    :coerce {:amount :int}                :refs {:orderId "Order" :customerId "Customer"}}
   {:entity "Order"        :plural "orders"        :id-prefix "square_ord"
    :fields [:locationId :customerId :state :totalAmount :currency] :required [:state :totalAmount]
    :coerce {:totalAmount :int}           :refs {:customerId "Customer"}}
   {:entity "Customer"     :plural "customers"     :id-prefix "square_cus"
    :fields [:givenName :familyName :emailAddress :phoneNumber] :required [:givenName :familyName]
    :coerce {}                            :refs {}}
   {:entity "Refund"       :plural "refunds"       :id-prefix "square_ref"
    :fields [:paymentId :amount :currency :status :reason] :required [:amount :currency]
    :coerce {:amount :int}                :refs {:paymentId "Payment"}}
   {:entity "CatalogObject" :plural "catalogobjects" :id-prefix "square_cat"
    :fields [:type :name :version]        :required [:type :name]
    :coerce {:version :int}               :refs {}}
   {:entity "Invoice"      :plural "invoices"      :id-prefix "square_inv"
    :fields [:orderId :status :invoiceNumber]  :required [:status :invoiceNumber]
...
(defn emit-facts
  "EAVT facts for one record: {\"square.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "square-compat" :tier tier :entities entities} 200])
...
(defn start! [] :square-compat/ready)
